...
# build the xnodes chain with Rollkit
ignite chain build
...
xnodesd keys add $KEY_2_NAME --keyring-backend test

# add these as genesis accounts
xnodesd add-genesis-account $KEY_NAME $TOKEN_AMOUNT --keyring-backend test
xnodesd add-genesis-account $KEY_2_NAME $TOKEN_AMOUNT --keyring-backend test
...
# xnodesd start --rollkit.aggregator true --rollkit.da_layer celestia --rollkit.da_config='{"base_url":"http://localhost:26659","timeout":60000000000,"fee":6000,"gas_limit":6000000}' --rollkit.namespace_id $NAMESPACE_ID --rollkit.da_start_height $DA_BLOCK_HEIGHT --rollkit.lazy_aggregator true